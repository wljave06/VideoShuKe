...
def get_accounts():
    ...
    try:
        ...
        for account in accounts:
            ...
            # 计算今日图片类别使用次数 (文生图 task_type=1 + 图生图 task_type=4)
            image_usage = JimengTaskRecord.select().where(
                (JimengTaskRecord.account_id == account.id) &
                (JimengTaskRecord.task_type.in_([1, 4])) &  # 1=文生图, 4=图生图
                (JimengTaskRecord.created_at >= today)
            ).count()

            # 计算今日视频类别使用次数 (图生视频 task_type=2 + 文生视频 task_type=5)
            video_usage = JimengTaskRecord.select().where(
                (JimengTaskRecord.account_id == account.id) &
            ...
            data.append({
                ...
                'today_usage': {
                    'image': image_usage,        # 图片类别：文生图+图生图 (10次/天)
                    'video': video_usage,        # 视频类别：图生视频+文生视频 (2次/天)
                    'digital_human': digital_human_usage  # 数字人类别 (1次/天)
                },
                'daily_limits': {
                    'image': 10,       # 图片每日限额10次
                    'video': 2,        # 视频每日限额2次
                    'digital_human': 1  # 数字人每日限额1次
                }
            })
...
def get_account_usage_stats():
    ...
    try:
        ...
        for account in accounts:
            ...
            from backend.models.models import JimengTaskRecord, JimengImg2ImgTask, JimengText2VideoTask
            ...
            # 统计总使用次数
            total_image = JimengTaskRecord.select().where(
                (JimengTaskRecord.account_id == account.id) &
                (JimengTaskRecord.task_type.in_([1, 4]))  # 1=文生图, 4=图生图
            ).count()

            total_video = JimengTaskRecord.select().where(
                (JimengTaskRecord.account_id == account.id) &
                (JimengTaskRecord.task_type.in_([2, 5]))  # 2=图生视频, 5=文生视频
            ).count()
            ...
            is_available = (image_usage < image_limit) and (video_usage < video_limit) and (digital_human_usage < digital_human_limit)
            ...
            stats.append({
                ...
                'remaining': {
                    'image': max(0, image_limit - image_usage),
                    'video': max(0, video_limit - video_usage),
                    'digital_human': max(0, digital_human_limit - digital_human_usage)
                },
                'total_usage': {
                    'image': total_image,
                    'video': total_video,
        ...
        # 计算总体统计数据
        total_today_image = sum(s['today_usage']['image'] for s in stats)
        total_today_video = sum(s['today_usage']['video'] for s in stats)
        total_today_digital_human = sum(s['today_usage']['digital_human'] for s in stats)

        total_remaining_image = sum(s['remaining']['image'] for s in stats)
        total_remaining_video = sum(s['remaining']['video'] for s in stats)
        ...
        return jsonify({
            ...
            'data': {
                ...
                'summary': {
                    ...
                    'today_usage': {
                        'image': total_today_image,
                        'video': total_today_video,
                        'digital_human': total_today_digital_human,
                        'total': total_today_image + total_today_video + total_today_digital_human
                    },
                    'remaining': {
                        'image': total_remaining_image,
                        'video': total_remaining_video,
                        'digital_human': total_remaining_digital_human,
                        'total': total_remaining_image + total_remaining_video + total_remaining_digital_human
                    }
                }
            },
            'message': '获取账号使用统计成功'
