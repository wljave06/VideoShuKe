...
from backend.models.models import JimengAccount, JimengTaskRecord, JimengFirstLastFrameImg2VideoTask
...
class JimengFirstLastFrameImg2VideoTaskManagerStatus(Enum):
    """即梦首尾帧图生视频任务管理器状态枚举"""
    STOPPED = "stopped"
    RUNNING = "running"
    PAUSED = "paused"
...
class JimengFirstLastFrameImg2VideoTaskManager:
    def __init__(self):
        self.platform_name = "即梦首尾帧图生视频"
        self.status = JimengFirstLastFrameImg2VideoTaskManagerStatus.STOPPED
    ...
    def start(self) -> bool:
        """启动首尾帧图生视频任务管理器"""
        if self.status == JimengFirstLastFrameImg2VideoTaskManagerStatus.RUNNING:
        ...
        self.status = JimengFirstLastFrameImg2VideoTaskManagerStatus.RUNNING
    ...
    def stop(self) -> bool:
        """停止首尾帧图生视频任务管理器"""
        if self.status == JimengFirstLastFrameImg2VideoTaskManagerStatus.STOPPED:
    ...
    def pause(self) -> bool:
        """暂停首尾帧图生视频任务管理器"""
        if self.status == JimengFirstLastFrameImg2VideoTaskManagerStatus.RUNNING:
            self.status = JimengFirstLastFrameImg2VideoTaskManagerStatus.PAUSED
            logger.info(f"首尾帧图生视频任务管理器已暂停")
            return True
        return False

    def resume(self) -> bool:
        """恢复首尾帧图生视频任务管理器"""
        if self.status == JimengFirstLastFrameImg2VideoTaskManagerStatus.PAUSED:
            self.status = JimengFirstLastFrameImg2VideoTaskManagerStatus.RUNNING
            logger.info(f"首尾帧图生视频任务管理器已恢复")
            return True
        return False
    ...
    def _worker_loop(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                self.status = JimengFirstLastFrameImg2VideoTaskManagerStatus.ERROR
                time.sleep(TASK_PROCESSOR_ERROR_WAIT)
                self.status = JimengFirstLastFrameImg2VideoTaskManagerStatus.RUNNING  # 自动恢复
    ...
    def _scan_and_process_tasks(self):
        ...
        try:
            ...
            # 查找排队中的首尾帧图生视频任务
            pending_tasks = JimengFirstLastFrameImg2VideoTask.select().where(
                JimengFirstLastFrameImg2VideoTask.status == 0
            ).order_by(JimengFirstLastFrameImg2VideoTask.create_at).limit(available_slots)
    ...
    def _submit_task_to_pool(self, task):
        ...
        try:
            ...
            future = global_task_manager.submit_task(
                ...
                task_id=task.id,
                task_type='首尾帧图生视频',
                prompt=f"首帧:{task.first_frame_image_path}, 尾帧:{task.last_frame_image_path}, 提示:{task.prompt}"
            )
    ...
    def _process_single_task(self, task):
        ...
        try:
            ...
            # 执行具体的任务处理逻辑
            result = run_async_safe(self._execute_first_last_frame_img2video_task(task))
            ...
            if result['success']:
                ...
                # 任务失败，但仍要更新cookies - 账号使用记录已在_execute_first_last_frame_img2video_task中处理
                if 'account_id' in result:
    ...
    async def _execute_first_last_frame_img2video_task(self, task) -> Dict:
        """
        执行即梦首尾帧图生视频任务的具体逻辑

        参数:
            task: JimengFirstLastFrameImg2VideoTask 对象

        返回值:
            Dict: 执行结果
        """

        logger.info(f"开始执行首尾帧图生视频任务，任务ID: {task.id}")
        logger.info(f"任务参数: first_frame='{task.first_frame_image_path}', last_frame='{task.last_frame_image_path}', prompt='{task.prompt}'")
        ...
        try:
            ...
            result = await executor.run(
                image_path=task.first_frame_image_path,
                last_frame_image_path=task.last_frame_image_path,  # 传入尾帧图片
                prompt=task.prompt,
                second=task.second,
            ...
            if result.code == 200 and result.data:
                # 更新账号使用次数
                await self.add_task_record(available_account.id, 2)  # 2=图生视频（包含首尾帧）
                ...
                error_msg = result.message or "即梦平台首尾帧图生视频失败"
            ...
            logger.error(f"即梦首尾帧图生视频任务执行异常: {str(e)}")
    ...
    async def add_task_record(self, account_id: int, task_type: int = 2,
        ...
        """添加任务记录到数据库 (task_type: 2=图生视频，包含首尾帧)"""
        try:
    ...
    def get_summary(self) -> Dict:
        ...
        try:
            ...
            pending_count = base_query.where(
                JimengFirstLastFrameImg2VideoTask.status == 0  # 排队中
            ).count()
            ...
            completed_count = base_query.where(
                JimengFirstLastFrameImg2VideoTask.status == 2  # 已完成
            ).count()

            failed_count = base_query.where(
                JimengFirstLastFrameImg2VideoTask.status == 3  # 失败
            ).count()
...
# 全局实例
jimeng_first_last_frame_img2video_task_manager = JimengFirstLastFrameImg2VideoTaskManager()
