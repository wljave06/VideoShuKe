...
class JimengText2VideoExecutor(BaseTaskExecutor):
    """即梦文本生成视频执行器"""
    ...
    async def perform_login(self, username: str, password: str) -> TaskResult:
        ...
        try:
            ...
            await asyncio.sleep(4)  # 增加等待时间，让登录请求有足够时间处理
    ...
    async def navigate_to_generation_page(self) -> TaskResult:
        ...
        try:
            ...
            self.logger.info("已跳转到文生视频生成页面并选择AI Video")
    ...
    async def select_video_ratio(self, ratio: str = "1:1") -> TaskResult:
        """选择视频比例（使用正确的HTML结构来定位单选按钮）"""
        try:
            self.logger.info("选择视频比例", ratio=ratio)
            
            # 根据提供的HTML代码，分辨率和比例选择是通过同一个按钮控制的
            # 按钮内部显示格式为 "比例/分辨率"，如 "16:9" 和 "720P"
            # 由于分辨率和比例在一个弹窗中，如果还没有点击按钮打开弹窗，需要先点击
            
            # 检查弹窗是否已经打开
            popup_open = await self.page.query_selector('div.lv-radio-group.radio-group-dl_J97')
            
            if not popup_open:
                # 如果弹窗未打开，点击选择按钮
                ratio_button_selector = 'button.lv-btn.lv-btn-secondary.lv-btn-size-default.lv-btn-shape-square.button-oBBmQ2'
                
                try:
                    await self.page.wait_for_selector(ratio_button_selector, timeout=5000)
                    await self.page.click(ratio_button_selector)
                    self.logger.info("已点击分辨率/比例选择按钮")
                    await asyncio.sleep(2)
                    
                    # 等待弹出的单选组出现
                    await self.page.wait_for_selector('div.lv-radio-group.radio-group-dl_J97', timeout=5000)
                    await asyncio.sleep(1)
                except Exception as btn_error:
                    self.logger.warning("未找到比例选择按钮，尝试备用选择器", error=str(btn_error))
                    # 备用选择器
                    backup_button_selector = 'button.lv-btn.lv-btn-secondary.lv-btn-size-default.lv-btn-shape-square'
                    buttons = await self.page.query_selector_all(backup_button_selector)
                    ...
                    if buttons:
                        ...
                        if target_button:
                            ...
                            self.logger.info("已点击比例选择按钮（通过备用选择器）")
                            await asyncio.sleep(2)
                            ...
                            self.logger.error("无法找到比例选择按钮")
                            return TaskResult(
                                code=ErrorCode.WEB_INTERACTION_FAILED.value,
                                data=None,
                                message="无法找到比例选择按钮",
                                error_details={"error": "无法找到比例选择按钮"}
                            )
            ...
            target_ratio = ratio_mapping.get(ratio, "1:1")  # 默认1:1
            
            # 现在在弹出的单选组中查找比例选项
            try:
                # 根据比例参数选择对应的单选按钮
                # 在HTML中，每个比例都有一个input[type="radio"]，其value为比例值
                ratio_radio_selector = f'input[type="radio"][value="{target_ratio}"]'
                
                # 使用JavaScript强制点击，因为元素可能被隐藏
                clicked = await self.page.evaluate(f'''
                    () => {{
                        const element = document.querySelector('{ratio_radio_selector}');
                        if (element) {{
                            element.click();
                            return true;
                        }}
                ...
                if clicked:
                    self.logger.info(f"已选择比例: {target_ratio}")
                else:
                    self.logger.error(f"无法选择比例: {target_ratio}")
                    return TaskResult(
                        code=ErrorCode.WEB_INTERACTION_FAILED.value,
                        data=None,
                        message=f"无法选择比例: {target_ratio}",
                        error_details={"error": f"无法找到或点击比例: {target_ratio}"}
                    )
                
                await asyncio.sleep(2)
                return TaskResult(code=ErrorCode.SUCCESS.value, data=None, message="视频比例选择成功")
                
            except Exception as option_error:
                self.logger.warning(f"精确比例选择失败: {str(option_error)}")
                
                # 尝试更通用的选择方法
                try:
                    # 查找包含目标比例的label元素
                    ratio_label = await self.page.query_selector(f'label:has-text("{target_ratio}")')
                    if ratio_label:
                        # 使用JavaScript点击label中的radio input
                        input_element = await ratio_label.query_selector('input[type="radio"]')
                        if input_element:
                            ...
                            ''', input_element)
                            if clicked:
                                self.logger.info(f"使用通用方法选择了比例: {target_ratio}")
                            else:
                                self.logger.error(f"JavaScript点击失败: {target_ratio}")
                                return TaskResult(
                                    code=ErrorCode.WEB_INTERACTION_FAILED.value,
                                    data=None,
                                    message=f"无法选择比例: {target_ratio}",
                                    error_details={"error": f"JavaScript点击失败: {target_ratio}"}
                                )
                        else:
                            # 如果没有找到input，尝试直接JavaScript点击label
                            clicked = await self.page.evaluate('''
                                (element) => {
                                    element.click();
                                    return true;
                            ...
                            ''', ratio_label)
                            if clicked:
                                self.logger.info(f"使用label点击选择了比例: {target_ratio}")
                            else:
                                self.logger.error(f"JavaScript label点击失败: {target_ratio}")
                                return TaskResult(
                                    code=ErrorCode.WEB_INTERACTION_FAILED.value,
                                    data=None,
                                    message=f"无法选择比例: {target_ratio}",
                                    error_details={"error": f"JavaScript label点击失败: {target_ratio}"}
                                )
                    else:
                        self.logger.error(f"无法找到比例选项: {target_ratio}")
                        return TaskResult(
                            code=ErrorCode.WEB_INTERACTION_FAILED.value,
                            data=None,
                            message=f"无法找到比例选项: {target_ratio}",
                            error_details={"error": f"无法找到比例选项: {target_ratio}"}
                        )
                except Exception as general_error:
                    self.logger.error(f"通用选择方法也失败: {str(general_error)}")
                    return TaskResult(
                        code=ErrorCode.WEB_INTERACTION_FAILED.value,
                        data=None,
                        message=f"无法找到比例选项: {target_ratio}",
                        error_details={"error": str(general_error)}
                    )
            ...
            self.logger.error("选择视频比例失败", error=str(e))
            # 不是致命错误，继续执行
            return TaskResult(code=ErrorCode.SUCCESS.value, data=None, message="视频比例选择完成（忽略错误）")
    ...
    async def wait_for_generation_complete(self, max_wait_time: int = 3600) -> TaskResult:
        ...
        try:
            ...
            while not self.generation_completed and time.time() - start_time < max_wait_time:
                ...
                # 检查是否有错误提示
                try:
                    error_message_element = await self.page.query_selector('div:has-text("Couldn\'t generate video now.Try again later.")')
                    if error_message_element:
                        error_text = await error_message_element.text_content()
                        self.logger.error("检测到视频生成失败提示", error_message=error_text)
                        return TaskResult(
                            code=ErrorCode.GENERATION_FAILED.value,
                            data=None,
                            message="视频生成失败: Couldn't generate video now. Try again later."
                        )
                except Exception as check_error:
                    self.logger.debug("检查错误提示时出错", error=str(check_error))
    ...
    async def execute(self, **kwargs) -> TaskResult:
        ...
        ratio = kwargs.get('ratio', '1:1')  # 添加比例参数
        username = kwargs.get('username')
        password = kwargs.get('password')
        cookies = kwargs.get('cookies')
        
        self.logger.info("开始执行文本生成视频任务", 
                        prompt=prompt, model=model, second=second, resolution=resolution, ratio=ratio)
        ...
        try:
            ...
            # 选择视频比例（在选择分辨率之后）
            ratio_result = await self.select_video_ratio(ratio)
