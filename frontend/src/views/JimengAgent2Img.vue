    ...
    <div class="page-header">
      <div class="header-content">
        <div class="title-section">
          ...
          <h1 class="page-title">即梦Agent模式</h1>
        </div>
...
import { agent2imgAPI } from '../utils/api'
  ...
  setup() {
    ...
    const fetchTasks = async () => {
      ...
      try {
        ...
        const response = await agent2imgAPI.getTasks(params)
    ...
    const fetchStats = async () => {
      try {
        const response = await agent2imgAPI.getStats()
        if (response.data.success) {
    ...
    const addTask = async () => {
      try {
        ...
        const response = await agent2imgAPI.createTask(taskData)
        if (response.data.success) {
    ...
    const deleteTask = async (taskId) => {
      try {
        const response = await agent2imgAPI.deleteTask(taskId)
        if (response.data.success) {
    ...
    const batchDeleteTasks = async () => {
      ...
      try {
        ...
        const response = await agent2imgAPI.batchDeleteTasks(taskIds)
        if (response.data.success) {
          ElMessage.success(response.data.message)
          selectedTasks.value = []
          refreshTasks()
    ...
    const retryTask = async (task) => {
      try {
        const response = await agent2imgAPI.retryTask(task.id)
        if (response.data.success) {
    ...
    const batchDownloadImages = async () => {
      ...
      try {
        ...
        const response = await agent2imgAPI.batchDownload(taskIds)
    ...
    const downloadSingleTask = async (task) => {
      try {
        ...
        const response = await agent2imgAPI.batchDownload([task.id])
    ...
    const batchDownloadImagesV2 = async () => {
      ...
      try {
        ...
        const response = await agent2imgAPI.batchDownloadV2(taskIds)
    ...
    const batchRetryFailedTasks = async () => {
      try {
        ...
        if (failedTaskIds.length > 0) {
          // 如果有选中的失败任务，只重试这些任务
          response = await agent2imgAPI.batchRetryTasks(failedTaskIds)
          if (response.data.success) {
          ...
          response = await agent2imgAPI.batchRetryTasks()
          if (response.data.success) {
            ElMessage.success(`已重新加入队列 ${response.data.data.retry_count} 个任务`)
          } else {
            ElMessage.error(response.data.message || '批量重试失败')
